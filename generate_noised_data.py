...
def save_noised_elec_data(elecdf, noise_lambda, seed):
    dirname = "lambda{:.2f}_seed{:d}".format(noise_lambda, seed)
    os.makedirs(dirname, exist_ok=True)
    noised_load_history = elecdf.copy(deep=True).astype("double")
    for zone in noised_load_history.columns:
        np.random.seed(seed+zone)
        if noise_lambda > 0.0:
            noise = np.random.laplace(loc=0.0, scale=noise_lambda, size=noised_load_history.values.shape[0])
            noised_load_history[zone] = noised_load_history[zone] + noise
    ...
    try:
        os.replace(os.path.join(dirname, "Load_history.csv"), os.path.join(dirname, "Load_history.csv.backup"))
    except Exception:
        pass
    noised_load_history.to_csv(os.path.join(dirname, "Load_history.csv"))


def do_generate_data(seed, noise_lambda):
    print(seed, noise_lambda)
    
    targetdf = elecdf[:-1]
    common_index = targetdf.index
    common_index = common_index.union(forecastdateindex)
    common_index = common_index.union(backcastdateindex)
    targetdf = targetdf.reindex(common_index)

    save_noised_elec_data(targetdf, noise_lambda, seed)
...
if __name__ == "__main__":
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        ...
        for noise_lambda in opt.lambdas:
            print(seed, noise_lambda)
            executor.submit(do_generate_data, seed, noise_lambda)